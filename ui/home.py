...
def show_decision_report():
    ...
    report = {
        ...
        "Break Even": f"{m.get('bep_units', 0):,.0f} units",
        "Net Cash": f"€{m.get('net_cash_position', 0):,.2f}"
    }
    st.table(pd.DataFrame(report.items(), columns=["Metric", "Value"]))
    pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", "B", 16); pdf.cell(200, 10, "Managers Lab Report", ln=True)
    pdf_out = pdf.output(dest="S").encode("latin-1")
    st.download_button("Download PDF", pdf_out, file_name="Report.pdf", mime="application/pdf")

def show_scenario_comparison():
    st.title("📊 Scenario Comparison")
    scens = st.session_state.get("saved_scenarios", {})
    if not scens: st.info("No saved scenarios."); return
    st.dataframe(pd.DataFrame([{"Name": k, **v} for k, v in scens.items()]))

def show_executive_dashboard():
    st.title("🏁 Executive Dashboard")
    m = st.session_state.get("metrics", {})
    st.metric("ROIC", f"{m.get('roic', 0)*100:.1f}%")
...
def run_home():
    ...
    c1.metric("Price", f"€{s.get('price', 100.0)}")
    ...
    with col_left:
        ...
        st.divider()
        scen_name = st.text_input("Scenario Name", value="Baseline", key="scen_input")
        if st.button("Save Scenario"):
            if "saved_scenarios" not in s: s.saved_scenarios = {}
            s.saved_scenarios[scen_name] = {"price": s.price, "metrics": m}
            st.success("Saved!")

    with col_right:
        st.subheader("🧠 Strategy Modules")
        if not s.get("baseline_locked"):
            st.info("Lock the baseline to see tools.")
        else:
