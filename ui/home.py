...
def show_executive_dashboard():
    ...
    if roic > 0.15: st.success("High return strategy.")
    elif roic > 0.05: st.warning("Moderate performance.")
    else: st.error("Low ROIC.")

    st.write(f"ROIC: **{roic*100:.1f}%** | Break-even: **{bep:,.0f}** | Cash: **€{cash:,.0f}**")
    ...
    if scenarios:
        ...
        rows = []
        for name, data in scenarios.items():
            rows.append({"Scenario": name, "ROIC": data.get("metrics", {}).get("roic", 0)})
        df = pd.DataFrame(rows)
...
def run_home():
    ...
    c1.metric("Contribution", f"€{p - vc:,.2f}")
    ...
    with col_left:
        ...
        scen_name = st.text_input("Scenario Name", value=s.get("scenario_name", "Baseline"), key="scenario_name_input")
        if st.button("Save Scenario", use_container_width=True):
            if "saved_scenarios" not in s: s.saved_scenarios = {}
            s.saved_scenarios[scen_name] = {"price": s.price, "volume": s.volume, "variable_cost": s.variable_cost, "fixed_cost": s.fixed_cost, "metrics": m}
