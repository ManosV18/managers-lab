...
def show_sidebar():
    ...
    with st.sidebar:
        ...
        st.session_state.annual_debt_service = st.number_input(
            "Annual Debt Service (€)", value=float(st.session_state.get('annual_debt_service', 0.0))
        )
        st.session_state.opening_cash = st.number_input(
            "Opening Cash (€)", value=float(st.session_state.get('opening_cash', 10000.0))
        )

        tax_percent = st.number_input(
            "Tax Rate (%)", value=float(st.session_state.get('tax_rate', 0.22)) * 100
        )
        ...
        if not st.session_state.get('wacc_locked', False):
            wacc_percent = st.number_input(
                "WACC (%)", min_value=0.0, max_value=100.0, value=st.session_state.wacc * 100, step=0.1
            )
        ...
        st.session_state.ar_days = st.number_input(
            "AR Days (Collection)", value=float(st.session_state.get('ar_days', 45.0))
        )
        st.session_state.inventory_days = st.number_input(
            "Inventory Days", value=float(st.session_state.get('inventory_days', 60.0))
        )
        st.session_state.ap_days = st.number_input(
            "AP Days (Payment)", value=float(st.session_state.get('ap_days', 30.0))
        )
        ...
        if st.button("🔄 Reset All Data", type="secondary", use_container_width=True):
            keys_to_reset = [
                ...
                'wacc', 'tax_rate', 'ar_days', 'inventory_days', 'ap_days',
                'annual_debt_service', 'opening_cash', 'baseline', 'baseline_locked'
            ]
