...
def render_cash_break_even_lab(
    ...
    st.subheader(
        "🧩 Turn Changes into Business Decisions"
    )

    st.caption(
        """
        The simulator above only tests the cash consequences.

        If you want to carry an actual business change into
        Managers Lab, add it as a Decision.

        Decisions are not executed here.
        """
    )
    ...
    if scenario_price != projected["price"]:

        decisions_to_add.append(
            DecisionFactory.price_change(
                decision_id=(
                    f"cash_be_price_{uuid4().hex}"
                ),
    ...
    if scenario_variable_cost != projected["variable_cost"]:

        decisions_to_add.append(
            DecisionFactory.variable_cost_change(
                decision_id=(
                    f"cash_be_vc_{uuid4().hex}"
                ),
                target_variable_cost=scenario_variable_cost,
            )
        )
    ...
    if scenario_fixed_opex != projected["fixed_opex"]:

        decisions_to_add.append(
            DecisionFactory.fixed_opex_change(
                decision_id=(
                    f"cash_be_opex_{uuid4().hex}"
                ),
                target_fixed_opex=scenario_fixed_opex,
            )
        )
    ...
    if scenario_interest != projected["interest"]:

        decisions_to_add.append(
            DecisionFactory.annual_interest_change(
                decision_id=(
                    f"cash_be_interest_{uuid4().hex}"
                ),
                target_annual_interest=scenario_interest,
            )
        )
    ...
    if scenario_principal != projected["principal"]:

        decisions_to_add.append(
            DecisionFactory.principal_payments_change(
                decision_id=(
                    f"cash_be_principal_{uuid4().hex}"
                ),
                target_principal_payments=scenario_principal,
            )
        )
    ...
    if scenario_volume != projected["volume"]:

        decisions_to_add.append(
            DecisionFactory.volume_change(
                decision_id=(
                    f"cash_be_volume_{uuid4().hex}"
                ),
                target_volume=scenario_volume,
            )
        )
    ...
    if decisions_to_add:
        ...
        st.write(
            f"**{len(decisions_to_add)} business change(s) detected.**"
        )

        for decision in decisions_to_add:
            st.caption(f"• {decision.name}")

        if st.button(
            "➕ Add Cash Scenario to Decision Plan",
            use_container_width=True,
        ):
            ...
            existing_changes = {
                ...
                for key in existing.changes.keys()
            }

            for decision in decisions_to_add:

                conflicting = (
                    set(decision.changes.keys())
                    & existing_changes
                )
                ...
                if conflicting:
                    ...
                    st.error(
                        "Cannot add the Cash Scenario because "
                        "one or more drivers are already represented "
                        "by another Decision: "
                        + ", ".join(conflicting)
                    )
                ...
                plan = plan.add(decision)

                existing_changes.update(
                    decision.changes.keys()
                )

            st.session_state.decision_plan = plan

            st.success(
                f"{len(decisions_to_add)} Decision(s) added to the Decision Plan."
            )
        ...
        st.caption(
            "No business assumptions have changed "
            "from the current decision projection."
        )
