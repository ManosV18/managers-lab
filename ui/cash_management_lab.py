...
#
# The Receivables Lab stores:
#
#   month_0_pct = collected in the same month
#   month_1_pct = collected next month
#   month_2_pct = collected two months later
#
# Example:
#
#   20% / 70% / 10%
#
# This is a timing assumption, NOT a cash amount.
# ---------------------------------------------------------------------
...
def _build_collection_cash_schedule(
    ...
    """
    Converts the 3-month collection timing assumption
    into actual six-month customer cash collections.
    ...
    The schedule is applied to:
      1. opening receivables
      2. each monthly revenue cohort

    This keeps the collection schedule as a cash-timing layer.
    AR days remains the canonical CompanyState driver.
    """

    annual_revenue = max(
        0.0,
        _as_float(annual_revenue),
    )
    ...
    for lag in range(3):
        ...
        if target_month < len(collections):
    ...
    for source_month in range(len(MONTHS)):
...
def build_cash_plan(
    ...
    if explicit_ar_collection_schedule is not None:

        ar_schedule = _build_collection_cash_schedule(
    ...
    dataframe.attrs["collection_cash_schedule"] = (
        ar_schedule
    )
...
def render_cash_management_lab(
    ...
    with st.expander(
        ...
        if schedule is not None:
            ...
            st.caption(
                "This schedule controls cash timing only. "
                "AR days remains the central working-capital driver."
            )
