...
def run_stage3():
    ...
    # Συγχρονισμός ονομάτων με την Engine
    wc_req = float(m.get('wc_requirement', 0.0))
    cash_reserve = float(m.get('net_cash_position', 0.0)) # Χρήση του net_cash από engine
    ccc = int(m.get('ccc', 0))
    ...
    c3.metric("Net Cash Position", f"€ {cash_reserve:,.0f}")

    # 

[Image of cash conversion cycle diagram showing inventory, receivables, and payables]
    ...
    if runway >= 100 or runway < 0: # Αν FCF > 0, το runway είναι άπειρο
    ...
    with col_prev:
        if st.button("⬅️ Back to Stage 2", use_container_width=True):
            ...
            st.rerun()
    with col_next:
        if st.button("Proceed to Stage 4 ➡️", use_container_width=True):
