...
# =========================================
def run_stage2():
    st.session_state.setdefault("debt", 0.0)
    st.session_state.setdefault("interest_rate", 0.05)
    st.session_state.setdefault("slow_moving_factor", 0.2)
    ...
    with col1:
        ...
        st.session_state.debt = st.number_input("Total Outstanding Debt (€)", min_value=0.0, value=float(st.session_state.debt))
        input_rate = st.number_input("Annual Interest Rate (%)", value=float(st.session_state.interest_rate*100))
        st.session_state.interest_rate = input_rate / 100

    with col2:
        st.subheader("Inventory & Cash Velocity")
        dio = st.number_input("Inventory Days (DIO)", value=st.session_state.inventory_days)
        slow_pct = st.slider("Slow-Moving / Buffer Stock (%)", 0,100,int(st.session_state.slow_moving_factor*100))
        st.session_state.slow_moving_factor = slow_pct/100
        dso = st.number_input("Receivable Days (DSO)", value=st.session_state.ar_days)
        dpo = st.number_input("Payable Days (DPO)", value=st.session_state.payables_days)
    ...
    c2.metric("Total Liquidity Friction", f"{st.session_state.liquidity_drain_annual:,.0f} €", delta=f"Incl. {slow_pct}% Slow-Stock", delta_color="inverse")
