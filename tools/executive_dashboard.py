...
def show_executive_dashboard():
    ...
    # 1. FETCH DATA (Ολοκληρωμένος συγχρονισμός)
    # Η sync_global_state καλεί εσωτερικά την calculate_metrics με τα 11 ορίσματα
    metrics = sync_global_state()
    s = st.session_state

    # Ανάκτηση τρεχόντων παραμέτρων με ασφάλεια
    ...
    with st.expander("Adjust Optimization Targets", expanded=True):
        ...
        # Προτείνουμε βελτίωση 20% ως default
...
opt_gap = optimized_metrics.get('wc_requirement', 0.0)
    ...
    m1, m2 = st.columns(2)
    m1.metric("Potential Cash Release", f"€ {cash_released:,.2f}", help="Instant liquidity injection.")
    m2.metric("Annual WACC Savings", f"€ {annual_savings:,.2f}", help="Permanent annual cost reduction.")

    # 6. VISUAL GAP ANALYSIS
    
    fig = go.Figure()
    fig.add_trace(go.Bar(name='Current', x=['Inv', 'Rec', 'Pay'], y=[curr_inv, curr_ar, -curr_ap], marker_color='#636EFA'))
    fig.add_trace(go.Bar(name='Optimized', x=['Inv', 'Rec', 'Pay'], y=[opt_inv, opt_ar, -opt_ap], marker_color='#00CC96'))
    fig.update_layout(barmode='group', template="plotly_dark", height=350, margin=dict(l=20, r=20, t=40, b=20))
    st.plotly_chart(fig, use_container_width=True)

    # 7. VERDICT & SYNC
    st.subheader("💡 Cold Analytical Verdict")
    if cash_released > 0:
        st.success(f"Execution liberates **€ {cash_released:,.2f}**. WACC gain: **€ {annual_savings:,.2f}/yr**.")
    ...
    if st.button("Finalize Strategy & Update Global Dials", use_container_width=True, type="primary"):
        ...
        st.success("Global model updated. Baseline recalculated.")
        st.rerun()
