...
def show_inventory_manager():
    ...
    with col1:
        ...
        M = st.number_input("Annual Demand (Units)", value=10000.0, format="%.2f")
        kf = st.number_input("Cost per Order (€)", value=600.0, format="%.2f")
        unit_p = st.number_input("Unit Purchase Price (€)", value=30.0, format="%.2f")
    ...
    with col2:
        ...
        interest_rate = st.number_input("Annual WACC/Interest (%)", value=5.0, step=0.1) / 100
    ...
    # EOQ Formula: sqrt((2 * Demand * OrderCost) / (UnitCost * (Interest + StorageRatio)))
    if r_disc == 0:
    ...
    if q_opt > 0:
        ...
        fig.update_layout(height=400, xaxis_title="Order Quantity (Units)", yaxis_title="Annual Cost (€)", margin=dict(l=20, r=20, t=20, b=20))
        st.plotly_chart(fig, use_container_width=True)

    # 6. VERDICT
    st.subheader("💡 Strategic Verdict")
    st.success(f"To minimize value destruction, the system should place **{num_orders:.1f} orders** per year of **{q_opt:,.0f} units** each. Any deviation from this point increases total costs via either excessive ordering work or capital stagnation.")
