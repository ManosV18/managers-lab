...
def pmt_basic(rate, nper, pv, fv=0, when=0):
    ...
    # Standard PMT formula: pv * (rate * (1 + rate)**nper) / ((1 + rate)**nper - 1)
    # Adjusting for payment at the beginning (when=1) or end (when=0)
    factor = (1 + rate)**nper
...
def loan_vs_leasing_ui():
    ...
    st.info("Analytical comparison of the total 15-year burden considering tax shields and financing structures.")
