...
def show_cash_fragility_index():
    ...
    # Το πραγματικό ετήσιο cash outflow περιλαμβάνει λειτουργικά ΚΑΙ δάνεια
    ...
    # Fragility Index: Αν > 1, ξεμένεις από μετρητά πριν προλάβεις να εισπράξεις από τον κύκλο σου.
    ...
    if cash_runway == 0: status = "NO RUNWAY"
    elif fragility_score > 1.0: status = "CRITICAL"
    elif fragility_score > 0.7: status = "VULNERABLE"
    else: status = "IMMUNE"
    ...
    if fragility_score > 1:
        st.error(f"**Structural Deficit:** Your Runway ({cash_runway:.1f} days) is shorter than your CCC ({ccc_days:.1f} days). The system is mathematically guaranteed to hit a liquidity wall.")
    else:
