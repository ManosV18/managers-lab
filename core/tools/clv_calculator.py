import streamlit as st
import pandas as pd
import plotly.graph_objects as go

def get_clv_data(purchases, margin_per_order, retention_years, discount, churn, realization, risk_p, cac):
    """
    Calculates NPV Customer Lifetime Value year-by-year
    """
    adj_disc = (float(discount) / 100) + (float(risk_p) / 100)
    churn_rate = float(churn) / 100
    cum_npv = -float(cac)
    data = []
    payback = None
    
    for t in range(1, int(retention_years) + 1):
        survival = (1 - churn_rate) ** t
        annual_flow = (float(purchases) * float(margin_per_order) * float(realization)) * survival
        discounted_flow = annual_flow / ((1 + adj_disc) ** t)
        cum_npv += discounted_flow
        
        data.append({"Year": t, "Cumulative_NPV": float(cum_npv)})
        if cum_npv >= 0 and payback is None:
            payback = t
            
    return pd.DataFrame(data), cum_npv, payback

def show_clv_calculator():
    st.header("👥 Executive CLV Simulator")
    
    s = st.session_state
    m = s.get("metrics", {})
    
    if not s.get('baseline_locked', False):
        st.warning("🔒 Access Denied: Please lock your Baseline in Home to enable CLV Unit Economics.")
        return 

    # --- 1. TRANSPARENCY: DATA AUDIT PANEL ---
    with st.expander("🔍 System Data Audit (Linked Parameters)", expanded=True):
        # Τραβάμε τα δεδομένα από το Home και το WACC
        linked_price = float(s.get('price', 0))
        linked_vc = float(s.get('variable_cost', 0))
        unit_contrib = linked_price - linked_vc
        linked_wacc = s.get('wacc_locked', 15.0) # Αν δεν υπάρχει WACC, default 15%

        audit_data = {
            "Parameter": ["Unit Price", "Unit Variable Cost", "Unit Contribution", "System WACC / Hurdle"],
            "Value": [f"${linked_price:,.2f}", f"${linked_vc:,.2f}", f"${unit_contrib:,.2f}", f"{linked_wacc:.2f}%"],
            "Source": ["Home Baseline", "Home Baseline", "Calculated Engine", "WACC Optimizer / Default"]
        }
        st.table(pd.DataFrame(audit_data))

    st.divider()

    # --- 2. SCENARIOS INPUT ---
    col_a, col_b = st.columns(2)
    with col_a:
        st.subheader("📊 Scenario A (Baseline)")
        purch_a = st.number_input("Purchases / Year (A)", value=4.0, key="pa")
        units_a = st.number_input("Units / Order (A)", value=1.0, key="ua")
        cac_a = st.number_input("CAC ($) (A)", value=150.0, key="caca")
        churn_a = st.slider("Annual Churn % (A)", 0, 100, 15, key="cha")

    with col_b:
        st.subheader("🚀 Scenario B (Optimization)")
        purch_b = st.number_input("Purchases / Year (B)", value=5.0, key="pb")
        units_b = st.number_input("Units / Order (B)", value=1.2, key="ub")
        cac_b = st.number_input("CAC ($) (B)", value=180.0, key="cacb")
        churn_b = st.slider("Annual Churn % (B)", 0, 100, 8, key="chb")

    # --- 3. NPV & RISK SETTINGS ---
    with st.expander("⚙️ Advanced NPV & Risk Adjustment"):
        c1, c2 = st.columns(2)
        
        # Σύνδεση με το WACC
        disc = c1.number_input("Base Discount Rate (WACC %)", value=float(linked_wacc))
        risk_p = c1.number_input("Risk Premium (%)", value=3.0, help="Additional risk for churn volatility.")
        real = c2.number_input("Realization Rate (0.0 - 1.0)", value=0.90)
        horizon = c2.slider("Analysis Horizon (Years)", 1, 15, 5)
        
        effective_rate = disc + risk_p
        st.caption(f"**Effective Risk-Adjusted Rate:** {effective_rate:.2f}%")

    # --- 4. EXECUTION ---
    margin_a = unit_contrib * units_a
    margin_b = unit_contrib * units_b

    df_a, final_a, pb_a = get_clv_data(purch_a, margin_a, horizon, disc, churn_a, real, risk_p, cac_a)
    df_b, final_b, pb_b = get_clv_data(purch_b, margin_b, horizon, disc, churn_b, real, risk_p, cac_b)

    # --- 5. RESULTS DASHBOARD ---
    st.divider()
    res_a, res_b = st.columns(2)
    
    with res_a:
        st.metric("NPV CLV (Scenario A)", f"${final_a:,.2f}")
        ratio_a = (final_a + cac_a) / cac_a if cac_a > 0 else 0
        st.write(f"LTV/CAC Ratio: **{ratio_a:.2f}x**")
        st.caption(f"Payback: {f'{pb_a} Years' if pb_a else 'Not Reached'}")

    with res_b:
        st.metric("NPV CLV (Scenario B)", f"${final_b:,.2f}", delta=f"${final_b - final_a:,.2f}")
        ratio_b = (final_b + cac_b) / cac_b if cac_b > 0 else 0
        st.write(f"LTV/CAC Ratio: **{ratio_b:.2f}x**")
        st.caption(f"Payback: {f'{pb_b} Years' if pb_b else 'Not Reached'}")

    # --- 6. VISUALIZATION ---
    

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_a['Year'], y=df_a['Cumulative_NPV'], name='Scenario A', line=dict(color='#EF553B', dash='dash')))
    fig.add_trace(go.Scatter(x=df_b['Year'], y=df_b['Cumulative_NPV'], name='Scenario B', line=dict(color='#00CC96', width=4)))
    fig.add_hline(y=0, line_dash="dot", line_color="grey")
    
    fig.update_layout(height=400, template="plotly_white", title="Cumulative Customer NPV Projection", xaxis_title="Year", yaxis_title="Cumulative Value ($)")
    st.plotly_chart(fig, use_container_width=True)

    # --- 7. VERDICT ---
    if ratio_b >= 3.0:
        st.success("🎯 **Verdict:** Scalable. LTV/CAC ≥ 3x.")
    elif ratio_b >= 1.0:
        st.warning("⚠️ **Verdict:** Low Efficiency.")
    else:
        st.error("🚨 **Verdict:** Value Destruction.")

    if st.button("⬅️ Return to Hub", use_container_width=True):
        st.session_state.flow_step = "home"
        st.rerun()
